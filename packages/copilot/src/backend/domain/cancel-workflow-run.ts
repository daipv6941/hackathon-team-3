...
const SELF = 'copilot.workflow.run.cancel.self';
const TENANT = 'copilot.workflow.run.cancel.tenant';
const INSTANCE = 'copilot.workflow.run.cancel.instance';
...
export async function cancelWorkflowRun(opts: CancelWorkflowRunOpts): Promise<void> {
  ...
  if (!perms.has(SELF) && !perms.has(TENANT) && !perms.has(INSTANCE)) {
    throw Object.assign(new Error('forbidden: cancel requires copilot.workflow.run.cancel.*'), {
      code: 'forbidden',
    });
  }
