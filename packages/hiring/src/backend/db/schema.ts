...
export const hiringCandidates = hiringSchema.table('candidates', {
  ...
  request_id: varchar('request_id', { length: 50 }).notNull(), // FK to hiringRequests.request_id
  jd_id: varchar('jd_id', { length: 50 }), // FK to hiringJobs.jd_id (can be NULL if JD not yet created)
  ...
  shortlisted_by: uuid('shortlisted_by'), // Identity.user_id
  shortlisted_at: timestamp('shortlisted_at', { withTimezone: true }).notNull().defaultNow(),

  // [AGENT OUTPUT] fields - populated by screen_cv_tool
  agent_recommendation: varchar('agent_recommendation', { length: 20 }), // Pass, Reject, Need More Info
  agent_fit_score: numeric('agent_fit_score', { precision: 5, scale: 2 }), // 0-100
  agent_fit_summary: text('agent_fit_summary'), // Why candidate is fit
  agent_gap_summary: text('agent_gap_summary'), // Why candidate has gaps
  agent_suggested_questions: text('agent_suggested_questions'), // Interview questions
  agent_shortlist_rank: integer('agent_shortlist_rank'), // 1, 2, 3... (generated by generate_report_tool)
  agent_screened_at: timestamp('agent_screened_at', { withTimezone: true }),
